...
var getStreamUrl = function (slug) { return __awaiter(void 0, void 0, void 0, function () {
    var url, response;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                url = "https://oploverz.fit/";
                return [4 /*yield*/, axios_1.default.get(url + slug).then(function (res) {
                        var html = res.data;
                        var $ = cheerio_1.default.load(html);
                        var thisParent = $(".postbody");
                        var childrenParent = $(".single-info");
                        var parentElem = $(".postbody").find(".spe");
                        var episode_title = thisParent.find("h1").text();
                        var stream_url = thisParent.find("iframe").attr("src");
                        var title = childrenParent.find("h2").text();
                        var status = filterSpan(parentElem, "Status:");
                        var studio = filterSpan(parentElem, "Studio:");
                        var duration = filterSpan(parentElem, "Durasi:");
                        var season = filterSpan(parentElem, "Season:");
                        ...
                        var genresText = genresElement
                            ...
                            .get();
                        var genres = genresText;
                        var desc = childrenParent.find(".mindes").text().replace(/\t|\n/g, "");
                        var index = [
                            {
